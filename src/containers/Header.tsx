...
export const Header = () => {
	...
	// select new language
	const selectLanguageHandler = (lang: langProp) => {
		setSelectedLanguage(lang)
		changeLanguage(lang.id);
	}

	// get langauge from session storage
	const getCurrentLangFromSession = () => {
		let session = new SessionHandler()
		let data: SessionType = session.getSession()

		let sessionLang: langProp = langList[0]

		if (data.lang !== "") {
			let res: any = langList.find(e => e.id === data.lang)
			sessionLang = res === undefined ? langList[0] : res
		}
		return sessionLang
	}
	...
	useEffect(() => {
		if (currentLanguage !== selectedLanguage.id) {
				...
				<div className="flex flex-1 items-center justify-end md:justify-between">
					...
					<div className="flex items-center gap-4">
						...
						<Dropdown
							...
							onSelect={(option) => selectLanguageHandler(option)}
